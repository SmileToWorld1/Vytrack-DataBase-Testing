...
public class ContactsStepDefs {
    ...
    //get information from database
        String query= "select c.first_name, c.last_name, e.email\n" +
                "from orocrm_contact c inner join orocrm_contact_email e\n" +
                "on c.id = e.owner_id\n" +
                "where e.email = 'shtlxeut@gmail.com'";

        //create the connection to qa3 env
       // DBUtils.createConnection();
        //get the data in java collections
        Map<String, Object> rowMap = DBUtils.getRowMap(query);
        String expectedFirstname = (String) rowMap.get("first_name");
        String expectedLastname = (String) rowMap.get("last_name");
        String expectedEmail = (String) rowMap.get("email");

        System.out.println("(expectedFirstname+\" \"+expectedLastname) = " + (expectedFirstname + " " + expectedLastname));
        System.out.println("expectedEmail = " + expectedEmail);
        //close connection
       // DBUtils.destroy();

        //assertion, compare UI vs DATABASE information
        Assert.assertTrue(actualFullname.contains(expectedFirstname)&&actualFullname.contains(expectedLastname));
